...
- **關鍵字搜尋**：課程編號、課程名稱、教師、系所皆可搜尋，支援多個關鍵字（空白分隔）。
...
```
src/
├─ views/HomeView.vue        # 首頁版面與接線
├─ components/home/          # 首頁元件：搜尋面板、搜尋結果、已選課程、課表、課程詳情／異動通知彈窗
├─ stores/                   # Pinia：課程資料、已選課程（localStorage）、搜尋與篩選狀態
├─ utils/                    # 純函式：時間／衝堂判斷、篩選比對、顯示格式、課表格子建構
├─ assets/ui.css             # 首頁元件共用的卡片／按鈕／標籤樣式
